...
- 🛡️ 本地优先，保护数据隐私
- 🌍 跨平台支持，包括 RISC-V 架构
- 🔧 双AI后端支持（Ollama和Candle）
- 💾 使用SQLite数据库存储对话
- 🎨 Element Plus UI组件与Vue 3
...
应用使用`config.yaml`文件进行配置：

```yaml
ai_model:
  model_type: "candle"  # "ollama"或"candle"
...
- **Ollama模式**: 使用外部Ollama服务器进行模型推理
- **Candle模式**: 使用内置的Rust原生AI推理与Hugging Face模型
...
应用界面同样支持多语言，可在`config.yaml`文件中配置：

```yaml
ui:
  language: "zh-CN"  # 更改为 "en-US" 以使用英文界面
```
